    ...
    const items: { icon?: ReactNode; key: string; children: ReactNode; divider: boolean }[] = [
      {
        ...
        divider: (Boolean(subs) && subs.length > 0) || (Boolean(collections) && collections.length > 0),
      },
    ]
    if (subs) {
              ...
              <DropdownMenu
                ...
                onAction={async key => {
                  if (key === 'open-substore') {
                    ...
                    try {
                      const sub = subs.find(sub => sub.name === key.toString().replace('sub-', ''))
                    ...
                    try {
                      const collection = collections.find(
                        collection => collection.name === key.toString().replace('collection-', ''),
                      )
                      await addProfileItem({
