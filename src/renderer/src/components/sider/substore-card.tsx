...
const SubStoreCard: React.FC<Props> = props => {
  ...
  const { substoreCardStatus = 'col-span-1', useSubStore = true, disableAnimation = false } = appConfig || {}
    ...
    >
      ...
      >
        ...
        <CardFooter className='pt-1'>
          <h3 className={`text-md font-bold ${match ? 'text-primary-foreground' : 'text-foreground'}`}>Sub-Store</h3>
        </CardFooter>
      </Card>
    </div>
  )
