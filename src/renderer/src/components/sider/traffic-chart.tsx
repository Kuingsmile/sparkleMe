...
export interface TrafficChartProps {
  data: { traffic: number; index: number }[]
  isActive: boolean
}
...
const TrafficChart: React.FC<TrafficChartProps> = props => {
  ...
  const gradientId = useMemo(() => `traffic-gradient-${isActive ? 'active' : 'inactive'}`, [isActive])
  ...
  return (
    ...
    >
      <AreaChart data={validData} margin={{ top: 50, right: 0, left: 0, bottom: 0 }}>
        ...
        <Area isAnimationActive={false} type='monotone' dataKey='traffic' stroke='none' fill={`url(#${gradientId})`} />
      </AreaChart>
    </ResponsiveContainer>
  )
}
...
export default React.memo(TrafficChart, (prevProps, nextProps) => {
  return prevProps.isActive === nextProps.isActive && JSON.stringify(prevProps.data) === JSON.stringify(nextProps.data)
})
