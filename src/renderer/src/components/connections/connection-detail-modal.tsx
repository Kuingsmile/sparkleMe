  ...
  return (
    ...
    >
      <div className='flex items-center gap-2 truncate'>
        <div className='truncate'>{displayName || (Array.isArray(value) ? value.join(', ') : value)}</div>
      </div>
    </SettingItem>
  )
}
      ...
      <ModalContent className='flag-emoji break-all'>
        ...
        <ModalBody>
          ...
          {connection.metadata.host && (
            <CopyableSettingItem title='主机' value={connection.metadata.host} prefix={['DOMAIN', 'DOMAIN-SUFFIX']} />
          )}
          ...
          {connection.metadata.processPath && connection.metadata.type !== 'Inner' && (
            <CopyableSettingItem title='进程路径' value={connection.metadata.processPath} prefix={['PROCESS-PATH']} />
          )}
          {connection.metadata.sourceIP && (
            <CopyableSettingItem title='来源IP' value={connection.metadata.sourceIP} prefix={['SRC-IP-CIDR']} />
          )}
          {connection.metadata.sourceGeoIP && connection.metadata.sourceGeoIP.length > 0 && (
            <CopyableSettingItem title='来源GeoIP' value={connection.metadata.sourceGeoIP} prefix={['SRC-GEOIP']} />
          )}
          {connection.metadata.sourceIPASN && (
            <CopyableSettingItem title='来源ASN' value={connection.metadata.sourceIPASN} prefix={['SRC-IP-ASN']} />
          )}
          {connection.metadata.destinationIP && (
            <CopyableSettingItem title='目标IP' value={connection.metadata.destinationIP} prefix={['IP-CIDR']} />
          )}
          {connection.metadata.destinationGeoIP && connection.metadata.destinationGeoIP.length > 0 && (
            <CopyableSettingItem title='目标GeoIP' value={connection.metadata.destinationGeoIP} prefix={['GEOIP']} />
          )}
          {connection.metadata.destinationIPASN && (
            <CopyableSettingItem title='目标ASN' value={connection.metadata.destinationIPASN} prefix={['IP-ASN']} />
          )}
          {connection.metadata.sourcePort && (
            <CopyableSettingItem title='来源端口' value={connection.metadata.sourcePort} prefix={['SRC-PORT']} />
          )}
          {connection.metadata.destinationPort && (
            <CopyableSettingItem title='目标端口' value={connection.metadata.destinationPort} prefix={['DST-PORT']} />
          )}
          {connection.metadata.inboundIP && (
            <CopyableSettingItem title='入站IP' value={connection.metadata.inboundIP} prefix={['SRC-IP-CIDR']} />
          )}
          {connection.metadata.inboundPort !== '0' && (
            <CopyableSettingItem title='入站端口' value={connection.metadata.inboundPort} prefix={['SRC-PORT']} />
          )}
          {connection.metadata.inboundName && (
            <CopyableSettingItem title='入站名称' value={connection.metadata.inboundName} prefix={['IN-NAME']} />
          )}
          {connection.metadata.inboundUser && (
            <CopyableSettingItem title='入站用户' value={connection.metadata.inboundUser} prefix={['IN-USER']} />
          )}
          {connection.metadata.dscp !== 0 && (
            <CopyableSettingItem title='DSCP' value={connection.metadata.dscp.toString()} prefix={['DSCP']} />
          )}
          {connection.metadata.remoteDestination && (
            <CopyableSettingItem title='远程目标' value={connection.metadata.remoteDestination} prefix={['IP-CIDR']} />
          )}
