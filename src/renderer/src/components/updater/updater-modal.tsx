    ...
    >
      <ModalContent className='h-full w-[calc(100%-100px)]'>
        ...
        <ModalBody className='h-full'>
          {updateStatus?.downloading && (
            <div className='space-y-3 mb-4'>
              ...
              <Progress value={updateStatus.progress} color='primary' size='sm' showValueLabel={false} />
              {updateStatus.error && <div className='text-danger text-sm'>{updateStatus.error}</div>}
            </div>
          )}
          {!updateStatus?.downloading && (
        ...
        <ModalFooter>
          ...
          {!updateStatus?.downloading && (
            <Button size='sm' color='primary' isLoading={downloading} startContent={<FiDownload />} onPress={onUpdate}>
              立即更新
            </Button>
          )}
        </ModalFooter>
